package net.server.cashshop.handlers;
...
public class CashShopOperation {
    ...
    public static void EnterCS(final CharacterTransfer transfer, final MapleClient c) {
        ...
        c.getSession().write(CSPacket.showNXMapleTokens(c.getCharacter()));
        doCSPackets(c);
        //loadCashShop(c);
//        doCSPackets(c);
    }
...
//            final CashItem item = CashItemFactory.getInstance().getMenuItem(sn);
            ...
            if (itemz != null) {
                ...
                c.getSession().write(CSPacket.showBoughtCSItem(itemz, item.getSN(), c.getAccountID()));
            } else {
                c.getSession().write(CSPacket.sendCSFail(0));
            }
...
//            c.getSession().write(CSPacket.sendWishList(chr, true));
            ...
            Item item = c.getCharacter().getCashInventory().findByCashId((int) slea.readLong());
...
//            System.out.println("action 15");
            ...
            if (item != null && item.getQuantity() > 0 && item.getUniqueId() > 0 && c.getCharacter().getCashInventory().getItemsSize() < 100) {
                ...
                c.getSession().write(CSPacket.showBoughtCSItem(item, item.getUniqueId(), c.getAccountID()));
                //warning: this d/cs
//                c.getSession().write(CSPacket.confirmToCSInventory(item, c.getAccID(), itemz.getSN()));
            ...
            chr.modifyCSPoints(toCharge, -item.getPrice(), false);
            c.getSession().write(CSPacket.showBoughtCSPackage(ccz, c.getAccountID()));
...
//        System.out.println("Scategory " + Scategory);
        if (Scategory == 103) {
            ...
            c.getSession().write(CSPacket.Favorite(c.getCharacter()));
...
//                System.err.println(category);
    ...
    public static void doCSPackets(MapleClient c) {
        ...
        c.getSession().write(CWvsContext.BuddylistPacket.updateBuddylist(c.getCharacter().getBuddylist().getBuddies()));
        c.getSession().write(CSPacket.showNXMapleTokens(c.getCharacter()));
